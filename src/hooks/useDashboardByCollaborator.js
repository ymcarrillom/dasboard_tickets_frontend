...
export function useDashboardByCollaborator(arg, limitArg) {
  const days = typeof arg === "number" ? arg : arg?.days ?? 30;
  const limit = typeof limitArg === "number" ? limitArg : arg?.limit ?? 10;

  const cacheKey = useMemo(() => `dash:byCollab:${days}:${limit}`, [days, limit]);
    ...
    (async () => {
      try {
        ...
        const data = await getJson(
          `/api/dashboard/by-collaborator?${qs.toString()}`,
          ctrl.signal
        );
        ...
        setError(e?.message || "Error cargando by-collaborator");
      } finally {
        setLoading(false);
        setFetching(false);
      }
...
export default useDashboardByCollaborator;
